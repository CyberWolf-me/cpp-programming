    ...
    int mobilecheck(char *m)
    {
        ...
        while (true)
        {
            ...
            if (n != 10)
            {
                ...
                cout << "Please Re-enter mobile number (10 digits): ";
                cin.getline(mobile, 11);
                strcpy(m, mobile); // Update the 'm' pointer to the new input
                continue;
            }
            for (int y = 0; y < strlen(m); y++)
            {
                ...
                {
                    cout << "Mobile number should only contain digits.\n";
                    cout << "Please Re-enter mobile number (10 digits): ";
                    cin.getline(mobile, 11);
                    strcpy(m, mobile); // Update the 'm' pointer to the new input
                    h = 0;             // Reset h because of invalid character
                    break;
                }
            }
            if (h == 10)
            ...
            {
                cout << "Mobile number is invalid (should be 10 digits).\n";
                cout << "Please Re-enter mobile number (10 digits): ";
                cin.getline(mobile, 11);
                strcpy(m, mobile); // Update the 'm' pointer to the new input
            }
...
public:
    ...
    int search(char *f, int choice)
    {
        ...
        {
            ifstream fin;
            fin.open("today.txt", ios::binary); // Opened in binary mode
        ...
        {
            ifstream fin;
            fin.open("pythonresult.txt", ios::binary); // Opened in binary mode
            fin.read((char *)this, sizeof(*this));
            while (!fin.eof())
            {
                if (!strcmp(f, name))
        ...
        else if (choice == 3) // Changed 'else if' to 'else if' for consistency
        {
            ifstream fin;
            fin.open("dsresult.txt", ios::binary); // Opened in binary mode
            fin.read((char *)this, sizeof(*this));
            while (!fin.eof())
            {
                if (!strcmp(f, name))
    ...
    int ifalreadygiven(char *f, int choice)
    {
        if (choice == 1)
        {
            ifstream fin;
    ...
    void inserttofile(char *name, int choice, int c = 0, int i = 0, int u = 0)
    {
        ...
        {
            ofstream file;
            file.open("today.txt", ios::app | ios::out | ios::binary); // Opened in binary mode
            getData(name, c, i, u);
        ...
        {
            ofstream file;
            file.open("pythonresult.txt", ios::app | ios::out | ios::binary); // Opened in binary mode
            getData(name, c, i, u);
            file.write((char *)this, sizeof(*this));
            file.close();
        }
        ...
        {
            ofstream file;
            file.open("dsresult.txt", ios::app | ios::out | ios::binary); // Opened in binary mode
            getData(name, c, i, u);
            file.write((char *)this, sizeof(*this));
            file.close();
        }
